...
function getWidgetExplanation(widget: string, dataset: Dataset): string {
  const pos = dataset.sentiment.positive;
  const neg = dataset.sentiment.negative;
  const neu = 100 - pos - neg;
  const theme1 = dataset.topThemes[0] ?? "key topic";
  const theme2 = dataset.topThemes[1] ?? "secondary topic";
  const theme3 = dataset.topThemes[2] ?? "emerging area";

  switch (widget) {
    case "Sentiment Breakdown":
      return `The **Sentiment Breakdown** for **${dataset.name}** shows **${pos}% positive**, **${neg}% negative**, and **${neu}% neutral** responses.\n\n**Why these numbers?**\n\n• **Positive (${pos}%)** is driven primarily by satisfied responses around **${theme3}** — respondents appreciated recent improvements in this area.\n• **Negative (${neg}%)** traces back to **${theme1}** and **${theme2}**, which are the two most cited pain points across all waves.\n• **Neutral (${neu}%)** represents responses that were informational or ambiguous — neither praise nor complaint.\n\nThe overall sentiment skew reflects the unresolved issues around **${theme1}**, which has been consistent across ${dataset.waves} waves. I'd recommend prioritising a response plan for that theme to move negative sentiment below the 20% threshold.`;

    case "Emotion Classification":
      return `The **Emotion Classification** widget breaks down the *quality* of sentiment in **${dataset.name}** — going deeper than just positive/negative to identify the specific emotional states behind responses.\n\n**Why this matters:**\n\n• **${dataset.emotionRate}%** of all ${dataset.responses.toLocaleString()} responses contained a classifiable emotion signal — that's a high signal rate, meaning respondents felt strongly enough to express emotion.\n• The dominant negative emotions — **Anger** and **Frustration** — point to **${theme1}** and **${theme2}** as the core unresolved issues. These emotions tend to escalate when problems recur across multiple interactions.\n• Positive emotions like **Satisfaction** and **Joy** cluster around **${theme3}**, suggesting that when experiences are smooth, they genuinely resonate.\n\nThe emotion distribution helps prioritise which issues need urgent attention (anger/frustration) vs. which positive experiences to reinforce (satisfaction/joy).`;

    case "AI Summary & Themes":
      return `The **AI Summary & Themes** card for **${dataset.name}** is generated by analysing patterns across all ${dataset.responses.toLocaleString()} responses.\n\n**How the summary was built:**\n\n1. Asa clustered responses by semantic similarity to identify recurring themes.\n2. The top 5 themes — led by **${theme1}**, **${theme2}**, and **${theme3}** — were ranked by frequency and emotional intensity.\n3. The summary text was generated to highlight the most actionable insight: the gap between the pain points and the areas of positive signal.\n\n**Active Risk Flags** are triggered when:\n• A theme appears in **>15%** of responses *and* carries a negative emotion.\n• Sentiment in a theme has worsened across waves.\n\nFor **${dataset.name}**, the flags around **${dataset.risks[0] ?? theme1}** were triggered because negative mentions increased between Wave ${dataset.waves - 1} and Wave ${dataset.waves}.`;

    case "Wave Sentiment Trend":
      return `The **Wave Sentiment Trend** chart shows how sentiment has shifted across the ${dataset.waves} collection waves for **${dataset.name}**.\n\n**Reading the chart:**\n\n• Each **green bar** represents the percentage of positive responses in that wave. Each **red bar** shows negative.\n• The bars are sized relative to each other — taller green = more positive signal in that period.\n\n**What the trend reveals:**\n\n• Wave 1 shows the highest positive sentiment — this is typical as respondents start with higher baseline goodwill.\n• By Wave ${dataset.waves}, positive sentiment has softened and negative has grown — this pattern almost always traces back to **${theme1}**, which tends to compound over time if unaddressed.\n• The widening gap between green and red from Wave 2 onward is the core insight: the issues are not resolving between waves, they're accumulating.\n\nI'd recommend reviewing the Wave ${dataset.waves} verbatims for **${theme1}** specifically to identify any new complaint patterns.`;

    case "Discussions":
      return `The **Discussions** widget surfaces the actual verbatim comments from **${dataset.name}**, organised by emotion and topic.\n\n**How it works:**\n\n• Each comment is tagged with a **detected emotion** (Anger, Satisfaction, Frustration, etc.) and a **topic** (one of the top themes).\n• Clicking an emotion block in the Emotion Classification widget filters this list to show only comments with that emotion — helping you drill into the specific language behind a data point.\n\n**What to look for:**\n\n• Comments tagged as **Anger** around **${theme1}** are the most actionable — these respondents are actively frustrated and at risk of escalation.\n• **Satisfaction** comments around **${theme3}** are worth preserving — they reveal what's working and can inform positive reinforcement strategies.\n• The **Confusion** and **Frustration** clusters often reveal process gaps or communication failures that aren't obvious in the quantitative data alone.\n\nThis widget is most powerful when used alongside the emotion filters to move from aggregate numbers to the actual human voice behind them.`;

    case "Dataset Health & Quality":
      return `The **Dataset Health & Quality** score for **${dataset.name}** reflects the integrity and completeness of the collected data.\n\n**How the score is calculated:**\n\n• **Response completeness** — what percentage of questions were answered by each respondent (partial responses lower the score).\n• **Response consistency** — whether answers within the same respondent are logically coherent.\n• **Wave coverage** — whether all ${dataset.waves} waves reached target sample size.\n• **Data freshness** — how recently the data was synced.\n\n**Current status:**\n\n${dataset.status === "active" ? `• The dataset is **active and healthy** — all waves are complete and data is current.\n• No anomalies or quality flags have been detected in the most recent sync.` : dataset.status === "processing" ? `• The dataset is **processing** — collection is ongoing and response targets may not yet be met. Sentiment figures will update as more responses arrive.` : dataset.status === "archived" ? `• The dataset is **archived** — all waves closed successfully with full samples.\n• Quality checks passed on the final sync.` : `• The dataset is **under review** — some quality flags have been raised. Treat any analysis as preliminary until the review is complete.`}\n\nHigh data quality means you can trust the sentiment and emotion numbers as statistically representative of the full respondent population.`;

    default:
      return `I can see you're looking at the **${widget}** widget in **${dataset.name}**.\n\nThis widget analyses the ${dataset.responses.toLocaleString()} responses collected across ${dataset.waves} waves to surface patterns in sentiment, emotion, and themes.\n\n**Key insight:** The overall **${neg}% negative sentiment** is primarily driven by **${theme1}** and **${theme2}** — these appear consistently across all waves.\n\nWould you like me to go deeper on any specific aspect of this data?`;
  }
}
...
function DatasetDetailPanel({ dataset, onAskPulse }: { dataset: Dataset; onAskPulse?: (widget: string, dataset: Dataset) => void }) {
...
function PersistentCopilot({
  ...
  widgetQuery?: { widget: string; dataset: Dataset; id: string } | null;
}) {
  ...
  // Inject widget explanation when Ask Pulse is triggered from DatasetDetailPanel
  useEffect(() => {
    if (!widgetQuery) return;
    const { widget, dataset: ds } = widgetQuery;
    setMessages((prev) => [
      ...prev,
      { id: makeId(), role: "user" as const, content: `Explain the ${widget} widget`, timestamp: "just now" },
    ]);
    setTyping(true);
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        { id: makeId(), role: "ai" as const, content: getWidgetExplanation(widget, ds), timestamp: "just now" },
      ]);
      setTyping(false);
    }, 900 + Math.random() * 400);
  // eslint-disable-next-line react-hooks/exhaustive-deps
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {pulseRightView === "dataset" && pulseRightDataset && (
                <DatasetDetailPanel
                  dataset={pulseRightDataset}
                  onAskPulse={(widget, ds) => {
                    addMsg({ role: "user", content: `Explain the ${widget} widget`, timestamp: "just now" });
                    aiTypingThen(900, { role: "ai", content: getWidgetExplanation(widget, ds), timestamp: "just now" });
                  }}
                />
              )}
...
function DatasetsPageInner() {
  ...
  const [activeMode, setActiveMode] = useState<"studio" | "pulse">(
    searchParams.get("view") === "pulse" ? "pulse" : "studio"
  );
  const [selectedDataset, setSelectedDataset] = useState<Dataset | null>(null);
  const [copilotCtx, setCopilotCtx] = useState<CopilotCtxData>({ context: "default" });
  const [leftView, setLeftView] = useState<"grid" | PanelMode>("grid");
  const [widgetQuery, setWidgetQuery] = useState<{ widget: string; dataset: Dataset; id: string } | null>(null);
    ...
    <>
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <div className="w-[36%] min-w-[340px] max-w-[500px] flex-shrink-0 flex flex-col overflow-hidden border-r border-[#E9EAEB] shadow-[8px_0_24px_rgba(10,13,18,0.06)]">
          <PersistentCopilot ctxData={copilotCtx} onOpenDetail={openDetail} widgetQuery={widgetQuery} />
        </div>
          ...
          {leftView !== "grid" ? (
            ...
            <div className="flex flex-col min-h-full">
              ...
              <div className="p-6 flex-1">
                {leftView === "dataset" && selectedDataset && (
                  <DatasetDetailPanel
                    dataset={selectedDataset}
                    onAskPulse={(widget, ds) => {
                      setWidgetQuery({ widget, dataset: ds, id: `${Date.now()}-${Math.random().toString(36).slice(2)}` });
                    }}
                  />
                )}
